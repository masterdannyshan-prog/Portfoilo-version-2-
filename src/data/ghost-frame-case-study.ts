export const ghostFrameCaseStudyTitle = "Ghost Frame - Full Case Study";

export const ghostFrameCaseStudyText = String.raw`Ghost Frame - UX/UI + Product Design Case Study

Overview
Ghost Frame is a browser-based image effects studio. You drop in a photo, pick one of 30 effects, adjust two sliders, and download the result. No account needed to use it. No app to install. No server processing your image - everything runs in your browser through the Canvas API.
...
* Tertiary text and labels: rgba(255, 255, 255, 0.4)
...
Hero
The hero has a video background and two pieces of text: "Turn Images Into Something Different" as the headline and a short descriptor below it. A demo card shows before and after switching.
...
How It Works
A 2x2 grid with four steps: Drop Your Image, Choose an Effect, Adjust the Sliders, Download or Save. Each cell has a large number, a title, and two lines of description.
...
Each card is divided into six horizontal strips (CSS grid, grid-template-rows: repeat(6, 1fr)). On hover, the strips animate out - they scale down on the Y axis, one after another with a staggered delay, revealing the underlying effect image and a title and arrow link.
...
Layout
Fixed 56px topbar with: logo on the left, filename display in the center, Reset / Save / Export buttons on the right. Below that: a three-column layout. Center: canvas area, takes up most of the screen. Right: 340px sidebar.
...
The 30 Effects
Six categories cover the full range.
Basic (6): Brightness, Contrast, Grain, Pixelate, Duotone, Blur.
Print and Graphic (6): Halftone, Dither, ASCII, Posterize, Threshold, Vignette.
Glitch and Distortion (7): RGB Shift, VHS, CRT, Pixel Sort, Chromatic Aberration, Scan Lines, Static.
...
Every effect is implemented as a Canvas API pixel manipulation function. The function takes source pixel data and writes to output pixel data. The two sliders map to intensity (0 to 1) and a second parameter (0 to 1) specific to that effect.
